...
let fileExists = (~message=?, filePath) =>
  assertion(~message?, ~operator="fileExists", (a, b) => a == b, Node.Fs.existsSync(filePath), true)
...
test("Generates sitemap", () => {
  ...
  fileContains(
    Node.Path.join([Node.Process.cwd(), "dist/sitemap.xml"]),
...
test("Adds information to page", () => {
  fileContains(
    ~message="Title",
    Node.Path.join([Node.Process.cwd(), "dist/index.html"]),
